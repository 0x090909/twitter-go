...
func getTweetTimeline(ctx context.Context, query string, maxTweetsNbr int, fetchFunc fetchTweetFunc) <-chan *TweetResultTimeline {
	channel := make(chan *TweetResultTimeline)
	go func(query string) {
		defer close(channel)
		var nextCursor string
		tweetsNbr := 0
		for tweetsNbr < maxTweetsNbr {
			select {
			case <-ctx.Done():
				channel <- &TweetResultTimeline{Error: ctx.Err()}
				return
			default:
			}
			...
			if err != nil {
				channel <- &TweetResultTimeline{Error: err}
				return
			}
			...
			for _, tweet := range tweets {
				...
				if tweetsNbr < maxTweetsNbr {
					nextCursor = next
					channel <- &TweetResultTimeline{Tweet: *tweet}
				} else {
					break
				}
				tweetsNbr++
